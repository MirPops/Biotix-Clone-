...
public class Cell1 : MonoBehaviour
{
    ...
    [SerializeField] private SpriteRenderer selectedRing;
    ...
    public Cell1 SelectCell(Player player)
    {
    ...
    public void Atack(Vector3 target)
    {
        ...
        {
            Vector3 randomPos = new Vector3(transform.position.x + Random.Range(-0.5f, 0.5f), transform.position.y + Random.Range(-0.5f, 0.5f), 0);

            GameObject miniCell = Instantiate(atackCells, randomPos, Quaternion.identity, parentOfMiniCells.transform);

            miniCell.GetComponent<MiniCell1>().atack(target, a);
        }
        amountCells -= cells;
        UpdateValue();
    }
